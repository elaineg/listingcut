...
Core flows:
1. Remove background client-side: user drags-and-drops or clicks to upload a JPEG/PNG; the page runs the free @imgly/background-removal WASM model in the browser (no server upload, no paid API) and shows a side-by-side before/after preview. While the model downloads (~40–80MB on first run) and runs (~5–20s), a visible status/progress indicator shows the current step (e.g. "downloading model…", "removing background…").
2. Download transparent PNG: after processing, a "Download PNG" button saves the cutout as a transparent PNG.
3. Marketplace-listing export (the differentiator): after the cutout appears, the user picks a platform preset — eBay 1600x1600, Etsy 2000x2000, or Poshmark square (1:1) — and clicks one button to download a JPEG of the subject composited centered on a solid white background at that exact canvas size. A preview thumbnail of the white-bg composite updates when the preset changes.

Success checks (a stranger can verify in a browser):
- Page loads with a drop zone reading something like "Drag & drop an image or click to upload", and a visible privacy claim stating the image never leaves the device / all processing happens in your browser — verifiable in <5 seconds without uploading anything.
- The three preset buttons/options labeled with "eBay 1600×1600", "Etsy 2000×2000", and "Poshmark" (square) are visible in the UI shell (may be disabled until a cutout exists) — verifiable without running inference.
...
- Selecting "eBay 1600×1600" and clicking the export button downloads a .jpg file that is exactly 1600×1600 pixels with white (255,255,255) corner pixels; Etsy export is exactly 2000×2000; Poshmark export is square (width equals height).
...
Out of scope:
- Manual touch-up / brush editing of the mask.
- Batch / multi-file processing and ZIP export.
- Custom background colors or background images (white only for MVP).
- Custom canvas sizes beyond the three presets.
